...
export class ToastService {
  ...
  private createToastContainer() {
    if (!document.getElementById('toast-container')) {
      ...
      container.className = 'toast toast-top toast-center z-50 w-1/4';
  ...
  private createToastElement(
    ...
    toast.classList.add('alert', alertClass, 'font-semibold', 'shadow-lg', 'flex', 'items-center', 'gap-3', 'cursor-pointer', 'justify-between');
  ...
  warning(message: string, duration?: number, avatar?: string, route?: string) {
    this.createToastElement(message, 'alert-warining', duration, avatar, route);
  }
